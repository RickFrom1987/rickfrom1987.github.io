...
async function run() {
  ...
  try {
    ...
    const isUserClickValue = await (
      await driver.findElement(webdriver.By.id("isUserClick"))
    ).getText();
    ...
    if (isUserClickValue.includes("true")) {
      await driver.executeScript(
        'browserstack_executor: {"action": "setSessionStatus", "arguments": {"status":"passed","reason": "isUserClick - works as expected!"}}'
      );
    } else {
      await driver.executeScript(
        'browserstack_executor: {"action": "setSessionStatus", "arguments": {"status":"failed","reason": "isUserClick - does not work as epexcted"}}'
      );
    }
  } catch (e) {
    console.log(e);
